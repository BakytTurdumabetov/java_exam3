...
public enum State {
    IN_STOCK("in_stock"){
        ...
        public void withdraw(Product product) throws Exception {
            throw new Exception("нельзя снять с торгов товар, который в них не учавтвует");
        }
    ...
    FOR_SALE("for_sale"){
        @Override
        public void startSale(Product product) throws Exception {
            throw new Exception("товар уже учатвует в торгах");
        }
        ...
        public void giveToTheWinner(Product product) throws Exception {
            if (product.getCurrentPrice() == product.getPrice()) {
                throw new Exception("нельзя товар отдать бесплатно");
            }
            if (product.getCurrentPrice() > product.getPrice()) {
                product.setStateObj(State.SOLD);
//                CodeGenerator generator = new CodeGenerator();
//                product.setHonoraryCode(generator.makeCode(String.valueOf(product.getId())));
